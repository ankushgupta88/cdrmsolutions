...
if(isset($_POST['category_submit'])){
  ...
  $slug = strtolower($_POST['slug']);
    ...
    $slug_query = "SELECT id,slug FROM `portfolios_cat` WHERE `slug` = '$slug'";
	$slug_result = mysqli_query($conn,$slug_query);	
	$slug_rows = mysqli_num_rows($slug_result);
	if($slug_rows >= 1){
        $slugExitsMsg = true; 
    } else {
        //insert new portfolios category
        $cat_insert_sql = "INSERT INTO portfolios_cat (name,slug,description,status)VALUES ('$name','$slug','$description','$status')";
          ...
          <div class="card card-primary">
            <form action="" method="POST" enctype="multipart/form-data">
              <div class="card-body">
                ...
                if($sucessMsg){
                  echo "<p class='msg-success'>Portfolio Category Added Successfully..</p>";
                } 
                if($unsucessMsg){
